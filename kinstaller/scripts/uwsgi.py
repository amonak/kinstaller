...
class Uwsgi(base.Installer):
    ...
    def _setup_kalabash_config(self):
        """Custom kalabash configuration."""
        dst = self._setup_config("kalabash")
        if package.backend.FORMAT == "deb":
            self._enable_config_debian(dst)
        else:
            system.add_user_to_group(
                "uwsgi", self.config.get("kalabash", "user"))
            utils.exec_cmd("chmod -R g+w {}/media".format(
                self.config.get("kalabash", "instance_path")))
            utils.exec_cmd("chmod -R g+w {}/pdfcredentials".format(
                self.config.get("kalabash", "home_dir")))
            pattern = (
